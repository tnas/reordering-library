...
C  CHECK THAT A TRIVIAL CASE HAS NOT BEEN ENTERED .
   ...
   20 FORMAT (/,/,/,20X,' ***KB07A*** NO NUMBERS TO BE SORTED ** ',
...
C later release of the code.
      ...
      REAL ZERO,ONE,TWO,SIXTN
      PARAMETER (ZERO = 0.0, ONE=1.0, TWO=2.0, SIXTN=16.0)
     ...
     +        IPERM,J,ISUP,JSUP,PAIR
      ...
      EXTERNAL MC60A,MC60B,MC60C,MC60D,MC60E,MC60F,MC60G
...
C Not even minimum required ... return a sufficient value.
      ...
      CALL MC60A(N,LIRN,IRN,ICPTR,ICON60,IW,INFO60)
...
C Use MC60F/FD to compute the profile etc for original matrix
         CALL MC60F(N,N,LIRN,IRN,ICPTR,IW(VARS),IW(IPERM),IW(IWORK),
     *               RINFO)
      ELSE
C Use MC60G/GD to compute statistics for original assembly order
         CALL MC60G(N,N,LIRN,IRN,ICPTR,IW(VARS),IW(IPERM),IW(IWORK),
     *               RINFO)
      END IF
...
C this
         CALL MC60B(N,LIRN,IRN,ICPTR,NSUP,IW(SVAR),IW(VARS),IW(IWORK))
      ELSE
...
C in the case when global priority function NOT supplied.
         ...
         CALL MC60C(N,NSUP,LIRN,IRN,ICPTR,IW(VARS),JCNTL,IW(PERMSV),
...
C variables from supervariable permutation
            IF (ICNTL(4).EQ.0) THEN
               CALL MC60D(N,NSUP,IW(SVAR),IW(VARS),IW(PERMSV),
     +                    PERM,IW(IWORK))
            ELSE
               DO 50 I = 1,N
...
C reduction that is not greater than ICNTL(7)% of the first reduction
      ...
      SUBROUTINE MC67A(N,LJCN,JCN,ROWPTR,PERM,LIW,IW,ICNTL,INFO,RINFO)
...
C Optionally compute profile for given permutation
      IF (ICNTL(6).NE.0)
     +   CALL MC67D(N,LJCN,JCN,ROWPTR,PERM,IW(INVPM),RINFO(4))
...
C Up/down
         DO 150 SWEEP = 1,MAXSWP
            ACGAIN = RINFO(1)
            CALL MC67C(N,LJCN,JCN,ROWPTR,IW(DIAG),IW(GAINS),IW(NUMFT),
     &                  PERM,IW(INVPM),IW(STEPS),IW(FIRST),RINFO(3),
     &                  ICNTL,NPASS)
            CALL MC67B(N,LJCN,JCN,ROWPTR,IW(SECOND),IW(NUMSEC),
...
C Down only
         CALL MC67B(N,LJCN,JCN,ROWPTR,IW(SECOND),IW(NUMSEC),IW(NUMFT),
...
C Set INFO(2) to hold number of sweeps performed
  ...
  200 CONTINUE
      ...
      SUBROUTINE MC67B(N,LJCN,JCN,ROWPTR,SECOND,NUMSEC,NUMFT,
...
C Jump to return if improvement is too small
  ...
  220 RETURN
      ...
      SUBROUTINE MC67C(N,LJCN,JCN,ROWPTR,DIAG,GAINS,NUMFT,
...
C Jump to return if improvement is too small
  ...
  220 RETURN
      ...
      SUBROUTINE MC67D(N,LJCN,JCN,ROWPTR,PERM,INVPM,FRONT)
