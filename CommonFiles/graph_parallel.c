...
void GRAPH_parallel_fixedpoint_static_BFS_old(const METAGRAPH* mgraph, int root, int** levels, const float percent_chunk)
{
...
void GRAPH_parallel_fixedpoint_static_BFS_original(const METAGRAPH* mgraph, int root, int** levels, const float percent_chunk)
{
	int node, n_nodes, ws_size, has_unreached_nodes, size_workset, tail, head;
	int* work_set;
	omp_lock_t lock;
...
// 				size_chunk = span;
// 				size_chunk = percent_chunk * span > 1 ? trunc(percent_chunk * span) : 1;
// 				head += size_chunk;
				omp_unset_lock(&lock);	
				
// 				size_chunk = percent_chunk * span > 1 ? trunc(percent_chunk * span) : 1;				
// 				size_chunk = ceil(percent_chunk * span);
// 				if (head + size_chunk > tail) head += span;
// 				else head += size_chunk;
// 				size_chunk = th_tail - th_head;
// 				printf("th_head: %d, th_tail: %d, percent: %f, size_chunk: %f\n", th_head, th_tail, percent_chunk * (th_tail - th_head),  size_chunk);fflush(stdout);
// 				if (size_chunk > 0) size_chunk = 1;
// 				printf("th_head: %d, th_tail: %d, size_chunk: %f\n", th_head, th_tail, size_chunk);fflush(stdout);
// 				if (size_chunk < 0) printf("th_head: %d, th_tail: %d, size_chunk: %f\n", th_head, th_tail, size_chunk);fflush(stdout);
// 				if (head + size_chunk > tail)
// 				{
// 					printf("th_head: %d, th_tail: %d, size_chunk: %f\n", th_head, th_tail, size_chunk);fflush(stdout);
// 				}
// 				
				...
				while ((!QUEUE_empty(work_set, th_head, th_tail)) && count_chunk < size_chunk)
				{
					active_node = QUEUE_deque(&work_set, ws_size, &th_head);
					QUEUE_enque(&active_chunk_ws, n_nodes, &active_tail, active_node);
					++count_chunk;
...
void GRAPH_parallel_fixedpoint_static_BFS_new(const METAGRAPH* mgraph, int root, int** levels, const float percent_chunk)
{
	int node, n_nodes, ws_size, has_unreached_nodes, tail, head, virtual_tail;
	int* work_set;
	omp_lock_t lock;
	...
	#pragma omp parallel 
	{
		...
		while ((!QUEUE_empty(work_set, head, tail)) || has_unreached_nodes > 0) 
		{
			...
			if (!QUEUE_empty(cache_work_set, cache_head, cache_tail))
			{
				...
				omp_set_lock(&lock);
				if (virtual_tail < tail) virtual_tail = tail;
				th_tail = virtual_tail;
				virtual_tail += size_chunk;
				omp_unset_lock(&lock);
