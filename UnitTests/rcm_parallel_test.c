...
test_result run_test_leveled_rcm_v1(const char* path_matrix_file, const int num_threads, int root)
{
	...
	time = get_time(); 
	Leveled_RCM_v1(matrix, &permutation, root);
	time = (get_time() - time)/100.0;
	result.time = time;
	...
	printf("Leveled RCM v1: Band/Env [ %ld / %ld => %ld / %ld ] Time [ %.6f ]\n",
		bandwidth_after, envelope_after, bandwidth, envelope, time); fflush(stdout);
...
test_result run_test_leveled_rcm_v2(const char* path_matrix_file, const int num_threads, int root)
{
	...
	time = get_time(); 
	Leveled_RCM_v2(matrix, &permutation, root);
	time = (get_time() - time)/100.0;
	result.time = time;
	...
	printf("Leveled RCM v2: Band/Env [ %ld / %ld => %ld / %ld ] Time [ %.6f ]\n",
		bandwidth_after, envelope_after, bandwidth, envelope, time); fflush(stdout);
...
test_result run_test_hsl_spectral(const char* path_matrix_file)
{
	long int bandwidth, envelope, bandwidth_after, envelope_after;
	int* permutation;
	double time;
	...
	time = get_time(); 
	REORDERING_SPECTRAL(matrix, &permutation);
	time = (get_time() - time)/100.0;
	result.time = time;
	...
	printf("HSL Espectral: Band/Env [ %ld / %ld => %ld / %ld ] Time [ %.6f ]\n",
		bandwidth_after, envelope_after, bandwidth, envelope, time); fflush(stdout);
...
test_result run_test_hsl_rcm(const char* path_matrix_file, int root)
{
	...
	printf("HSL RCM: Band/Env [ %ld / %ld => %ld / %ld ] Time [ %.6f ]\n",
		bandwidth_after, envelope_after, bandwidth, envelope, time); fflush(stdout);
